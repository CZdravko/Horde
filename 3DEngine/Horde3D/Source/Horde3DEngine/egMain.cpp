...
DLLEXP void h3dShowOverlays(const float *verts, int vertCount, float colR, float colG, float colB, float colA, uint32 materialRes, int flags) {
...
DLLEXP ResHandle h3dCreateTexture(const char *name, int width, int height, int fmt, int flags) {
	TextureResource *texRes = new TextureResource(safeStr(name, 0), (uint32) width, (uint32) height, 1, (TextureFormats::List) fmt, flags);
...
DLLEXP bool h3dGetRenderTargetData(ResHandle pipelineRes, const char *targetName, int bufIndex, int *width, int *height, int *compCount, void *dataBuffer, int bufferSize) {
	if (pipelineRes != 0) {
		...
		return ((PipelineResource *) resObj)->getRenderTargetData(safeStr(targetName, 0), bufIndex, width, height, compCount, dataBuffer, bufferSize);
	} else {
...
DLLEXP void h3dGetNodeTransform(NodeHandle node, float *tx, float *ty, float *tz, float *rx, float *ry, float *rz, float *sx, float *sy, float *sz) {
	SceneNode *sn = Modules::sceneMan().resolveNodeHandle(node);
...
DLLEXP void h3dSetNodeTransform(NodeHandle node, float tx, float ty, float tz, float rx, float ry, float rz, float sx, float sy, float sz) {
	SceneNode *sn = Modules::sceneMan().resolveNodeHandle(node);
...
DLLEXP void h3dGetNodeAABB(NodeHandle node, float *minX, float *minY, float *minZ, float *maxX, float *maxY, float *maxZ) {
	SceneNode *sn = Modules::sceneMan().resolveNodeHandle(node);
	APIFUNC_VALIDATE_NODE(sn, "h3dGetNodeAABB", APIFUNC_RET_VOID);

	Modules::sceneMan().updateNodes();
	if (minX != 0x0)
		*minX = sn->getBBox().min.x;
	if (minY != 0x0)
		*minY = sn->getBBox().min.y;
	if (minZ != 0x0)
		*minZ = sn->getBBox().min.z;
	if (maxX != 0x0)
		*maxX = sn->getBBox().max.x;
	if (maxY != 0x0)
		*maxY = sn->getBBox().max.y;
	if (maxZ != 0x0)
		*maxZ = sn->getBBox().max.z;
}
...
DLLEXP void h3dSetupModelAnimStage(NodeHandle modelNode, int stage, ResHandle animationRes, int layer, const char *startNode, bool additive) {
	...
	((ModelNode *) sn)->setupAnimStage(stage, (AnimationResource *) animRes, layer, safeStr(startNode, 0), additive);
}
...
DLLEXP void h3dSetModelAnimBias(NodeHandle modelNode, int stage, float biasTransX, float biasTransY, float biasTransZ,
		float biasRotX, float biasRotY, float biasRotZ, float biasRotW) {
	SceneNode *sn = Modules::sceneMan().resolveNodeHandle(modelNode);
	APIFUNC_VALIDATE_NODE_TYPE(sn, SceneNodeTypes::Model, "h3dSetModelAnimBias", APIFUNC_RET_VOID);

	((ModelNode *) sn)->setAnimBias(stage, biasTransX, biasTransY, biasTransZ, biasRotX, biasRotY, biasRotZ, biasRotW);
}
...
DLLEXP NodeHandle h3dAddMeshNode(NodeHandle parent, const char *name, ResHandle materialRes, int batchStart, int batchCount, int vertRStart, int vertREnd) {
	SceneNode *parentNode = Modules::sceneMan().resolveNodeHandle(parent);
	...
	MeshNodeTpl tpl(safeStr(name, 0), (MaterialResource *) matRes, (unsigned) batchStart, (unsigned) batchCount, (unsigned) vertRStart, (unsigned) vertREnd);
...
DLLEXP NodeHandle h3dAddLightNode(NodeHandle parent, const char *name, ResHandle materialRes, const char *lightingContext, const char *shadowContext) {
	SceneNode *parentNode = Modules::sceneMan().resolveNodeHandle(parent);
	...
	LightNodeTpl tpl(safeStr(name, 0), (MaterialResource *) matRes, safeStr(lightingContext, 1), safeStr(shadowContext, 2));
...
DLLEXP NodeHandle h3dAddEmitterNode(NodeHandle parent, const char *name, ResHandle materialRes, ResHandle particleEffectRes, int maxParticleCount, int respawnCount) {
	SceneNode *parentNode = Modules::sceneMan().resolveNodeHandle(parent);
	...
	EmitterNodeTpl tpl(safeStr(name, 0), (MaterialResource *) matRes, (ParticleEffectResource *) effRes, (unsigned) maxParticleCount, respawnCount);
