...
bool AnimationResource::load(const char *data, int size) {
	...
	// Load animation data
	uint32 numEntities;
	memcpy(&numEntities, pData, sizeof(uint32));
	pData += sizeof(uint32);
	memcpy(&_numFrames, pData, sizeof(uint32));
	pData += sizeof(uint32);
...
//			printf("%s \t %f\t %f\t %f\t %f\t ; T =  %f\t %f\t %f\n", name, frame.rotQuat.x, frame.rotQuat.y,
...
AnimResEntity *AnimationResource::findEntity(uint32 nameId) {
	...
	while (first <= last) {
		...
		if (nameId > _entities[mid].nameId)
			first = mid + 1;
		else if (nameId < _entities[mid].nameId)
			last = mid - 1;
		else
			return &_entities[mid];
	}
...
bool AnimationController::setupAnimStage(int stage, AnimationResource *anim, int layer, const string &startNode, bool additive) {
	...
	curStage.animNode = 0x0;

	curStage.animNode = _nodeList[0].node;

//	curStage.biasTranslation = Vec3f(0,0,0);
//	curStage.biasRotation = Quaternion(0,0,0,1);

	for (size_t i = 0, s = _nodeList.size(); i < s; ++i)
		mapAnimRes((uint32) i, stage);

//	for (size_t i = 0, s = _nodeList.size(); i < s; ++i) {
//		if (curStage.startNodeNameId != 0) {
//			curStage.animNode = _nodeList[i].node;
//			while (curStage.animNode != 0x0) {
//				if (hashName(curStage.animNode->getANName().c_str()) == _animStages[stage].startNodeNameId) {
//					break;
//				}
//				curStage.animNode = curStage.animNode->getANParent();
//			}
//		} else {  //Find the topmost joint
//			printf("curStage.startNodeNameId == 0");
//
//		}
//	}
...
bool AnimationController::setAnimBias(int stage, float biasTransX, float biasTransY, float biasTransZ, float biasRotX, float biasRotY, float biasRotZ, float biasRotW) {
	if ((unsigned) stage > _animStages.size()) {
		Modules::setError("Invalid stage in h3dSetModelAnimParams");
		return false;
	}
	...
	curStage.biasRotation = Quaternion(biasRotX, biasRotY, biasRotZ, biasRotW);
	curStage.biasTranslation = Vec3f(biasTransX, biasTransY, biasTransZ);

	return true;
}

bool AnimationController::animate() {
	if (!_dirty || _activeStages.empty())
		return false;
	...
	if (Modules::config().gatherTimeStats)
	...
	for (size_t i = 0, si = _nodeList.size(); i < si; ++i) {
		...
		//TODO: BIAS
		Vec3f biasTranslation(0, 0, 0);
		Quaternion biasRotation(0, 0, 0, 1);

		// Fast path
		if (Modules::config().fastAnimation && _activeStages.size() == 1) {
			uint32 firstStage = _activeStages[0];

			const AnimStage &curStage = _animStages[firstStage];
			if (_nodeList[i].node == curStage.animNode) {  //_nodeList[i].node == curStage.animNode
				//Apply BIAS
				biasTranslation = curStage.biasTranslation;
				biasRotation = curStage.biasRotation;
			}
			...
			if (animEnt != 0x0 && !animEnt->frames.empty()) {
				...
				if (animEnt->frames.size() == 1)
					frame = 0;  // Animation compression

				nodeTransVec = biasTranslation + animEnt->frames[frame].transVec;
				nodeRotQuat = biasRotation * animEnt->frames[frame].rotQuat;
				nodeScaleVec = animEnt->frames[frame].scaleVec;

				Matrix4f mat(Math::NO_INIT);
				Matrix4f::fastMult43(mat, Matrix4f(nodeRotQuat), Matrix4f::ScaleMat(nodeScaleVec.x, nodeScaleVec.y, nodeScaleVec.z));
				Matrix4f::fastMult43(_nodeList[i].node->getANRelTransRef(), Matrix4f::TransMat(nodeTransVec.x, nodeTransVec.y, nodeTransVec.z), mat);

//				_nodeList[i].node->getANRelTransRef() = Matrix4f(biasRotation) * animEnt->frames[frame].bakedTransMat;//.translate(biasTranslation.x, biasTranslation.y, biasTranslation.z);
//				_nodeList[i].node->getANRelTransRef().translate(biasTranslation.x, biasTranslation.y, biasTranslation.z);
			}
			continue;
		}
		...
		for (size_t j = 0, sj = _activeStages.size(); j < sj; ++j) {
			...
			if (_nodeList[i].node == curStage.animNode) {  //
				//Apply BIAS
				biasTranslation = curStage.biasTranslation;
				biasRotation = curStage.biasRotation;
			}
			...
			if (animEnt == 0x0 || layerWeightSum < Math::Epsilon)
				continue;
			...
			if (numFrames > 0) {
				...
				//HERE WE NEED TO APPLY BIAS
				rotQuat = biasRotation * rotQuat;
				transVec = biasTranslation + transVec;
				...
				if (curStage.additive) {
					...
					if (nodeUpdated) {
						...
						Vec3f fullScaleVec(nodeScaleVec.x * (scaleVec.x / firstFrame.scaleVec.x), nodeScaleVec.y * (scaleVec.y / firstFrame.scaleVec.y),
