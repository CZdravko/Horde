...
SceneGrapgPlugin::SceneGrapgPlugin() {
...
void GameEngine::SceneGrapgPlugin::dllLoadGamePlugin(void) {
	GameLog::logMessage(" Initialization of Horde3D SceneGrapgPlugin! ");
	printf(" Initialization of Horde3D SceneGrapgPlugin! ");
...
void GameEngine::SceneGrapgPlugin::dllUnloadGamePlugin(void) {
...
void GameEngine::SceneGrapgPlugin::dllLoadScene(const char* sceneFile) {
	GameLog::logMessage("Setting Horde3D Configuration");
	GameLog::logMessage(" loading scene in  SceneGrapgPlugin! ");
	printf("  loading scene in SceneGrapgPlugin! ");
	...
	if (!engineSettings.isEmpty()) {
		...
		bool sRGBLinearization = _stricmp(engineSettings.getAttribute("sRGBLinearization", "0"), "true") == 0 || _stricmp(engineSettings.getAttribute("sRGBLinearization", "0"), "1") == 0;
		...
		bool loadTextures = _stricmp(engineSettings.getAttribute("loadTextures", "1"), "1") == 0 || _stricmp(engineSettings.getAttribute("loadTextures", "1"), "true") == 0;
		...
		const XMLNode& engineSettings(scene.getChildNode("EngineConfig"));
		if (!engineSettings.isEmpty()) {
			bool fastAnim = _stricmp(engineSettings.getAttribute("fastAnimation", "false"), "true") == 0 || _stricmp(engineSettings.getAttribute("fastAnimation", "0"), "1") == 0;
		...
		bool wireframeMode = _stricmp(engineSettings.getAttribute("wireframeMode", "0"), "1") == 0 || _stricmp(engineSettings.getAttribute("wireframeMode", "0"), "true") == 0;
		GameLog::logMessage("WireframeMode: %s", wireframeMode ? "enabled" : "disabled");
		h3dSetOption(H3DOptions::WireframeMode, wireframeMode);

		bool debugViewMode = _stricmp(engineSettings.getAttribute("debugViewMode", "0"), "1") == 0 || _stricmp(engineSettings.getAttribute("debugViewMode", "0"), "true") == 0;
		GameLog::logMessage("DebugViewMode: %s", debugViewMode ? "enabled" : "disabled");
		h3dSetOption(H3DOptions::DebugViewMode, debugViewMode);

		bool dumpFailedShaders = _stricmp(engineSettings.getAttribute("dumpFailedShaders", "0"), "1") == 0 || _stricmp(engineSettings.getAttribute("dumpFailedShaders", "0"), "true") == 0;
		...
		bool gatherTimeStats = _stricmp(engineSettings.getAttribute("gatherTimeStats", "1"), "1") == 0 || _stricmp(engineSettings.getAttribute("gatherTimeStats", "1"), "true") == 0;
	...
	if (!stereoSettings.isEmpty()) {
		...
		SceneGraphManager::instance()->setStereoscopyParams((float) atof(stereoSettings.getAttribute("eyeOffset", "0.05")), (float) atof(stereoSettings.getAttribute("strabismus", "2.5")),
				(float) atof(stereoSettings.getAttribute("focalLength", "1")));
...
void GameEngine::SceneGrapgPlugin::dllRegisterLuaStack(lua_State* L) {

}

const char* GameEngine::SceneGrapgPlugin::getName() {
	return "SceneGrapgPlugin";
}
