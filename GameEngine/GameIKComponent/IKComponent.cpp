...
void IKComponent::executeEvent(GameEvent *event)
{
	...
	case GameEvent::IK_CREATEIKANIM:
		{
			...
			{
				m_anim = new IKAnim(m_config, Joint::getInstance(hordeID, data->endEffectorName), Joint::getInstance(hordeID, data->stopName), Horde3D::Vec3f(data->targetX, data->targetY, data->targetZ) );
	...
	case GameEvent::IK_SETPARAMI:
		{
			IKData* data = static_cast<IKData*>(event->data());
			m_config->setParamI((IK_Param::List)data->ikparam, data->ikparam_valuei);
		}
		break;
	case GameEvent::IK_SETPARAMF:
		{
			IKData* data = static_cast<IKData*>(event->data());
			m_config->setParamF((IK_Param::List)data->ikparam, data->ikparam_valuef);
		}
		break;
	case GameEvent::IK_GETPARAMI:
		{
			IKData* data = static_cast<IKData*>(event->data());
			data->ikparam_valuei = m_config->getParamI((IK_Param::List)data->ikparam);
		}
		break;
	case GameEvent::IK_GETPARAMF:
		{
			IKData* data = static_cast<IKData*>(event->data());
			data->ikparam_valuef = m_config->getParamF((IK_Param::List)data->ikparam);
		}
...
void IKComponent::loadFromXml(const XMLNode* node)
{
	...
	//Parsing config settings from scene file
	m_config = new Config();


	if( node->nChildNode("IKConfig") == 1)
	{
		m_config->loadFromXml(node->getChildNode("IKConfig"));
	}
	...
	if( leye_name == 0 || strcmp ( "", leye_name ) == 0
	 ...
	 GameLog::errorMessage( "IKComponent: information for gaze initialization missing" );
		...
		m_gaze = new Gaze( Joint::getInstance(hordeID, neck_name), Joint::getInstance(hordeID, leye_name), Joint::getInstance(hordeID, reye_name), m_config );
}
...
{
	m_anim = new IKAnim( m_config, Joint::getInstance(modelHandle, endEffectorName), Joint::getInstance(modelHandle, stopName), Horde3D::Vec3f(targetX, targetY, targetZ) );
...
{
	CCD aCCD( Joint::getInstance(modelHandle, endEffectorName), Joint::getInstance(modelHandle, stopName), Horde3D::Vec3f(targetX, targetY, targetZ), m_config );
	return aCCD.execute();
}
