...
IKAnim::IKAnim(Config* settings, Joint *endEffector, Joint *chainEnd, Horde3D::Vec3f target)
: m_ccd( endEffector,chainEnd,target,settings), m_chain(0), m_config(settings)
{
	//intializations
	m_numFrames = m_config->getParamI( IK_Param::NumAnimFrames_I );
...
{
	Horde3D::Vec3f p,r,s;
	int version = (int)m_config->getParamF(IK_Param::H3DAVersion_F);
