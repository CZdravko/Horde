...
KeyframeAnimationJob::KeyframeAnimationJob(KeyframeAnimComponent* owner, KeyframeAnimationResource* anim, AnimationSetup* command) :
		m_animation(anim), m_stageID(command->Stage), m_layerID(command->Layer), m_speed(command->Speed), m_currentTimeWeight(-1), m_jobID(0), m_owner(owner), m_paused(false), m_pauseTimestamp(0), m_startFrame(
				0), m_biasTransX(0), m_biasTransY(0), m_biasTransZ(0), m_biasRotX(0), m_biasRotY(0), m_biasRotZ(0), m_biasRotW(1) {
	...
	m_startFrame = command->StartFrame;
	...
	if (command->Speed == 0) {
		...
		if (command->Duration == 0) {
			...
			command->Duration = (m_animation->Frames - m_startFrame) / m_speed;
		...
		// If duration was less than zero the user wants an endless animation with the default speed (command->Speed == 0)
			...
			// Speed is calculated from duration and available frames (play it one time for the specified duration)
			command->Speed = m_speed = (m_animation->Frames - m_startFrame) / command->Duration;
	...
	} else { //FOUND an empty slot so use it
		*iter = this;
...
#endif
	printf("Removing animation job: %d\n", m_jobID);
	// Disable animation
...
void KeyframeAnimationJob::update(const float timestamp) {
	...
	if (m_timeline.empty() || m_currentTimeWeight + 1 >= timeweights)
		return;
	...
	if (m_currentTimeWeight < 0) {
		...
		if (m_timeline.front().Timestamp > timestamp)
			return; // ... return
		//  otherwise we have to activate it
		//AND set BIAS!
		else {
			if (m_owner->m_charCont != 0x0)
				m_owner->m_charCont->jobStarted(m_jobID);
			m_animation->activate(true, m_stageID, m_layerID, m_timeline.front().Weight < 0);
			m_animation->setBias(m_stageID, m_biasTransX, m_biasTransY, m_biasTransZ, m_biasRotX, m_biasRotY, m_biasRotZ, m_biasRotW);
		}
		// if startFrame not 0 update animation resource
		if (m_startFrame > 0) {
			m_animation->update(animTime * m_speed + m_startFrame, m_stageID, 1);
		}
	...
	while (m_currentTimeWeight + 1 < timeweights) {
		...
		if (tw2.Timestamp >= timestamp) {
			...
			m_animation->update(animTime * m_speed + m_startFrame, m_stageID, interpolatedWeight);
			break;
		}
		//Keyframe reached

		++m_currentTimeWeight;
		if (m_currentTimeWeight + 1 < timeweights) {			//not the end of animation
			if (m_owner->m_charCont != 0x0){

				m_owner->m_charCont->keyframeReached(m_jobID, m_stageID, (int)(animTime * m_speed + m_startFrame));
			}
		}else{
			if (m_owner->m_charCont != 0x0)
							m_owner->m_charCont->jobFinished(m_jobID);
		}
#ifdef PRINT_ANIMATION_INFO
...
void KeyframeAnimationJob::setStartFrame(const float startFrame) {
	m_startFrame = startFrame;
}

void KeyframeAnimationJob::setAnimBias(float biasTransX, float biasTransY, float biasTransZ, float biasRotX, float biasRotY, float biasRotZ, float biasRotW) {
	m_biasTransX = biasTransX;
	m_biasTransY = biasTransY;
	m_biasTransZ = biasTransZ;
	m_biasRotX = biasRotX;
	m_biasRotY = biasRotY;
	m_biasRotZ = biasRotZ;
	m_biasRotW = biasRotW;

//	m_animation->setBias(m_stageID, biasTransX, biasTransY, biasTransZ, biasRotX, biasRotY, biasRotZ, biasRotW);
}
